...
def filtro_media(A):
    A = A[A > 0]  # Exclude zero values to avoid log(0)
    #S = np.exp(np.mean(np.log(A))) Way 1
    S = A.prod()**(1.0/A.size) # Way 2
    return S
...
filtro= ndi.generic_filter(noise_img,filtro_media, [5,5])
